...
if( ! class_exists( 'Postembeds_Customizer' ) ) {

    class Postembeds_Customizer
    {
        public function __construct()
        {
            if ( is_admin() ) {
                add_action( 'admin_menu', [$this, 'postembeds_load_menu'] );

                // @since 1.0.1 - Move settings from vg_post_embeds_settings to postembeds_settings option
                if ( $postembeds_settings = get_option( 'vg_post_embeds_settings' ) ) {
                    update_option( 'postembeds_settings', $postembeds_settings );
                ...
                add_action( 'wp', function()
                {
                    ...
                    $enabled_post_types = self::postembeds_supported_post_types();
                    ...
                    if( $wp_query->is_embed && in_array( $wp_query->posts[0]->post_type, $enabled_post_types ) ) {
                        ...
                        if( $this->postembeds_single_setting( 'style', 'social-bird' ) !== 'default' ) {
                            ...
                            add_action( 'embed_content_meta', [$this, 'postembeds_footer_meta'] );
                            add_action( 'embed_content_meta', [$this, 'postembeds_comments_meta'] );
                            add_action( 'embed_footer', [$this, 'postembeds_footer_embed'] );
                            add_action( 'embed_footer', [$this, 'postembeds_load_scripts'] );
                        }

                        add_action( 'embed_head', [$this, 'postembeds_load_styles'] );

                        add_filter( 'embed_template', [$this, 'postembeds_load_embed_template'], 9999 );
                        add_filter( 'body_class', [$this, 'postembeds_set_body_classes'], 9999 );
                        add_filter( 'embed_site_title_html', [$this, 'postembeds_site_logo'], 9999 );
                        add_filter( 'excerpt_more', [$this, 'postembeds_continue_reading'], 9999 );

                        // Custom hooks
                        add_action( 'postembeds_datetime', [$this, 'postembeds_date_time'] );
                        add_action( 'postembeds_readmore', [$this, 'postembeds_readmore'] );
                        add_action( 'postembeds_author', [$this, 'postembeds_author'] );
                    }
                } );
            }
        }
         ...
         */
        private function postembeds_supported_post_types()
        {
            $settings           = get_option( 'postembeds_settings' );
         ...
         */
        public function postembeds_load_embed_template( $template )
        {
            $style      = $this->postembeds_single_setting( 'style', 'social-bird' );
            $template   = file_exists( POSTEMBEDS_DIR . '/templates/' . $style . '.php' )
                            ? POSTEMBEDS_DIR . '/templates/' . $style . '.php'
                            : POSTEMBEDS_DIR . '/templates/default.php';

            // Post not found or wrong URL
            if( is_404() ) {
                $template = POSTEMBEDS_DIR . '/templates/404.php';
            }
         ...
         */
        public function postembeds_load_styles()
        {
         ...
         * Add admin menu
         */
        public function postembeds_load_menu()
        {
            ...
            add_submenu_page(
                ...
                'postembeds',
                [
                    $this, 'postembeds_load_settings_page'
                ]
            );
         ...
         */
        public function postembeds_load_settings_page()
        {
            ?>
            <div class="wrap">
                <h1>
                ...
                // Avatar
                $avatar                     = isset( $settings['avatar'] ) ? esc_html( $settings['avatar'] ) : 'favicon';
                ...
                <form method="post">
                    <?php wp_nonce_field( 'postembeds_settings_nonce', 'postembeds_settings_nonce_field' ) ?>
                    ...
                    <table class="form-table">
                        <tr>
                            ...
                            <td>
                                ...
                                <p class="description">
                                    <?php _e( 'The post embed theme.', 'post-embeds' ); ?>
                                </p>
                            </td>
                        </tr>
                        <tr>
                            <th scope="row">
                                <?php _e( 'Avatar', 'post-embeds' ) ?>
                            </th>
                            <td>
                                <label>
                                    <select name="avatar">
                                        <option value="favicon"
                                            <?php echo $avatar === 'favicon' ? ' selected' : '' ?>
                                        >
                                            <?php esc_attr_e( 'Site favicon', 'post-embeds' ) ?>
                                        </option>
                                        <option value="author"
                                            <?php echo $avatar === 'author' ? ' selected' : '' ?>
                                        >
                                            <?php esc_attr_e( 'Author profile picture', 'post-embeds' ) ?>
                                        </option>
                                    </select>
                                </label>
                                <p class="description">
                                    <?php
                                    _e( 'You can replace the favicon with this setting.',
                                        'post-embeds'
                                    );
                                    ?>
                                </p>
         ...
         * @since 1.0.0
         */
        private function postembeds_save_settings()
        {
            ...
            {
                if ( ! isset( $_POST['postembeds_settings_nonce_field'] )
                    || ! wp_verify_nonce(
                        sanitize_key( $_POST['postembeds_settings_nonce_field'] ),
                        'postembeds_settings_nonce'
                    )
                ) {
                    $this->postembeds_notify_msg(
                ...
                // Avatar
                $settings['avatar']                     = isset( $_POST['avatar'] ) && ! empty( $_POST['avatar'] ) ? sanitize_text_field( $_POST['avatar'] ) : 'favicon';
                ...
                update_option( 'postembeds_settings', $settings );
         ...
         *
         * @since 1.0.0
         * @param string $setting Single setting stored in postembeds_settings option
         * @param string $default Default value when not saved in database
         *
         * return mixed
         */
        public function postembeds_single_setting( $setting, $default )
        {
            $all        = get_option( 'postembeds_settings' );
         ...
         */
        public function postembeds_notify_msg( $text, $type = 'success' )
        {
            if( $type === 'success' ) {
                $class = 'updated fade';
            } else {
         ...
         * @since 1.0.0
         */
        public function postembeds_set_body_classes( $classes )
        {
            unset($classes);
         ...
         * @since 1.0.0
         */
        public function postembeds_footer_embed()
        {
        	if ( is_404() ) {
        		return;
        	}
         ...
         * @since 1.0.0
         */
        public function postembeds_footer_meta()
        {
            if ( is_404() ) {
        		return;
        	}
         ...
         * @since 1.0.0
         */
        public function postembeds_site_logo( $site_title )
        {
            global $post;
            ...
            if ( $avatar === 'author' ) {
                // Gravatar
                $site_title = sprintf(
                    '<a href="%s" target="_top"><img src="%s" width="52" height="52" alt="" class="pe-embed-site-icon" /></a>',
                    esc_url( get_author_posts_url( $post->post_author ) ),
                    esc_url( get_avatar_url( 
                        get_the_author_meta( 'email', $post->post_author ), array( 'size' => 64 ) 
                    ) )
                );
            } else {
                // Favicon
         ...
         */
        public function postembeds_continue_reading( $more_string )
        {
            global $post;
         ...
         * @since 1.0.0
         */
        public function postembeds_comments_meta()
        {
         ...
         * @since 1.0.0
         */
        public function postembeds_load_scripts()
        {
        	wp_print_inline_script_tag(
        		file_get_contents( POSTEMBEDS_DIR . '/assets/js/script.js' )
        	);
        }
         ...
         * @since 1.0.0
         */
        public function postembeds_date_time()
        {
            global $post;
            ...
            if( $display_date && $display_time ) {
                ...
                $datetime_order = $this->postembeds_single_setting( 'datetime_order', 'date-time' );
         ...
         * @since 1.0.0
         */
        public function postembeds_readmore()
        {
            global $post;
            ...
            $readmore_text              = $this->postembeds_single_setting( 'readmore_text', '' );
         ...
         * @since 1.0.0
         */
        public function postembeds_author()
        {
            global $post;
